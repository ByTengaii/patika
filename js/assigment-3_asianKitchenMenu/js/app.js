...
//Show all menu when load the page;
document.addEventListener('DOMContentLoaded', createMenu(menu));

// Add filter option to button
buttons.forEach((btn) => {
    btn.addEventListener('click',menuFilter);
})

function menuFilter(event){
    id = event.target.id;

    let result = menu.filter((element) => element.category.toLowerCase() == id); //create filtered object
    if (id === 'all')
        result = menu;
    createMenu(result);
